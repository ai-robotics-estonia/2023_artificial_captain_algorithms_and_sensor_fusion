    ...
    def __getitem__(self, index):
        ...
        while len(negative_images) < 32:
            ...
            if img0_tuple[1] == img1_tuple[1]:
                ...
                # Keeping the image_path of negative images
...
# PyTorch object of image dataset dir
folder_dataset_test = dset.ImageFolder(root=Config.testing_dir)
siamese_dataset = Siamese_Triplet_Test(imageFolderDataset=folder_dataset_test,
                                       transform=transforms, should_invert=False)  # Initializing data parser class
test_dataloader = DataLoader(siamese_dataset, num_workers=12,
                             batch_size=1, shuffle=False)  # PyTorch data parser obeject
...
for i in tqdm(range(total-1)):
    ...
    for j in range(len(negatives)):  # Iterate over multiple negative samples
        ...
        # Creating a [anchor, positive, negative] sample to be passed onto the model for testing
        concatenated = torch.cat((anc, pos, neg), 0)
        output1, output2, output3 = net(Variable(anc).cuda(
        )*gaussian_mask, Variable(pos).cuda()*gaussian_mask, Variable(neg).cuda()*gaussian_mask)
